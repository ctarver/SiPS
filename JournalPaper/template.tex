...
Whenever discontiguous signals pass through a nonlinearity, they intermodulate creating intermodulation distortion (IMD) components throughout the nearby spectrum which could interfere with other users or with a device's own receiver in a frequency-division duplexing scenario \cite{Park13}. 
...
Digital predistortion is a signal processing technique that requires sampling the output of the PA to learn the effective nonlinearities and then apply some sort of inverse nonlinaeities before the PA with the goal of canceling the PAs nonlinearities. 
...
This would lead to their intermodulation distortion products being spaced even farther apart so that the observation bandwidth necessary to mitigate the spurious emissions becomes unpractical for many analog-to-digital converters (ADCs) and radio-frequency (RF) downconverters \cite{S.A.BassamOct.2011}. 

Carrier aggregation has made its way from the standards down to commercial implementations being used with many networks providing support for it and system-on-chips such as the Snapdragon 835 supporting three downlink carriers and two uplink carriers. 
Although carrier aggregation is becoming more commonplace, most DPD solutions do not consider the case of or provide a solution for the intermodulation products. 
...
To combat this, the authors introduced a subband DPD method in \cite{ICASSP2014}. 
